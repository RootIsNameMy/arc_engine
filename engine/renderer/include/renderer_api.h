...
namespace arc {
enum DrawType { Triangle, Line };
class RendererAPI {
 public:
  enum class API { None, OpenGL, Vulkan, OpenGL_ES, WebGL };
  ...
  virtual void SetDrawType(DrawType draw_type_) = 0;
...
class RenderCommand : public Caller {
  ...
  typedef void(VoidMemberDrawTypeFn)(void *, DrawType);
  ...
  static VoidMemberDrawTypeFn *s_set_draw_type_fn_;
 ...
 public:
  ...
  inline static void Clear() { s_clear_fn_(s_renderer_api_); }
  ...
  inline static void SetDrawType(DrawType draw_type) {
    s_set_draw_type_fn_(s_renderer_api_, draw_type);
  }
